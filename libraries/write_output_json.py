...
def write_output_json(row): #single output
	def str_form(raw_list):
		form_list = ["{:.2e}".format(i) if str(i)[:4] == '0.00' else str(i) for i in raw_list]
		return form_list
	...
	pred['results'].append({'tumor_type': re.sub('\.',' ',row['Pred1'].values[0]), 'confidence': str_form([row['Conf1'].values[0]])})
	pred['results'].append({'tumor_type': re.sub('\.',' ',row['Pred2'].values[0]), 'confidence': str_form([row['Conf2'].values[0]])})
	pred['results'].append({'tumor_type': re.sub('\.',' ',row['Pred3'].values[0]), 'confidence': str_form([row['Conf3'].values[0]])})
