...
def top_shap_single(model, test_x, test_y, colnames):
	...
	shap_values = pd.concat([shap_values, pd.DataFrame(test_x)], ignore_index=True)
...
def format_gdd_output(single_data, res, allprobs, shap):
	...
	final_res = [single_data.SAMPLE_ID.values[0], single_data.Diagnosed_Cancer_Type.values[0], single_data.Classification_Category.values[0], res.pred_label.values[0], prob_form[0], res.pred_label.values[1], prob_form[1], res.pred_label.values[2], prob_form[2]] + allprobs_form
...
if __name__ == "__main__":
	...
	# This checkpoint stores a full model object, not only raw weights.
	gdd_model = torch.load('ensemble.pt', map_location=torch.device(device), weights_only=False)
