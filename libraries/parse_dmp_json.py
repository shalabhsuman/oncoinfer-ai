...
def parse_dmp_json(data):
	...
	try:
		...
		if data['meta-data']['gender'] == 0 or data['meta-data']['gender'] == 1:
			...
			features['Gender_F'] = 1
		features['MSI_SCORE'] = float(data['msi']['msi_score']) if data['msi']['msi_score'] != '' else -1
	...
	try:
		...
		hotspot = pd.read_csv("final_hotspot_list.csv", index_col = 0) 
		...
		for mut in data['snp-indels']: #TODO - make sure all of these name bugs are fixed
			...
			if re.search('p14ARF|p16INK4A', mut['gene_id']):
				continue
			elif re.search('TERT', mut['gene_id']):
				if re.search('^(promoter|5pFlank|upstream)$', mut['variant_class']):
			...
			# Mark this gene as mutated, and also create an entry for the specific mutation
				...
				if not re.search('^Missense|^nonsynonymous_SNV|^missense|^5pFlank|^upstream|^In_Frame', mut['variant_class']):
		...
		if gene_panel not in impact_gene_panel.keys():
			print(gene_panel + ' Unsupported Gene Panel, Using Default Expected Coverage')
			gene_panel = 'IM7'
	...
	try:
		...
		with open('cytoband_table.txt', 'r') as f:
